...
DLUAU_API bool dluau_samemeta(lua_State* L, int idx, const char* tname);
DLUAU_API int16_t dluau_defaultuseratom(const char* key, size_t len);
...
DLUAU_API void dluau_init(lua_State* L);
...
DLUAU_API int dluau_run(const luauxt_RunOptions* opts);
...
DLUAU_API lua_CompileOptions* dluau_compileoptions;
